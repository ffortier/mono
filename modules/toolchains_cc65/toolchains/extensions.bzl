...
load("@toolchains_cc65//toolchains/private:common.bzl", "arch_from_rctx", "os_from_rctx")
...
def _is_compatible_with_host(rctx, os, arch):
    return os_from_rctx(rctx) == os and arch_from_rctx(rctx) == arch
...
def _impl(mctx):
    ...
    for attrs in root_module.tags.binary_snapshot:
        ...
        if attrs.urls[0].endswith(".deb"):
            if len(root_module.tags.common_snapshot) != 1:
                fail("cc65 binary snapshot .deb requires exactly one common snapshot")

            common_attrs = root_module.tags.common_snapshot[0]
            ...
            cc65_binaries(
                ...
                exec_os = os,
                exec_arch = arch,
                common_urls = common_attrs.urls,
                common_sha256 = common_attrs.sha256,
                common_strip_prefix = common_attrs.strip_prefix,
            )
        else:
            cc65_binaries(
                name = "cc65_%s_%s" % (os, arch),
        ...
        if _is_compatible_with_host(mctx, os, arch):
            has_host_binaries = True

    if len(root_module.tags.bootstrap_from_sources) == 1 and not has_host_binaries:
